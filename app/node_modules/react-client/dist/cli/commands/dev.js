...
export default async function dev() {
                ...
                if (id.endsWith('.css')) {
                    ...
                    if (hasConfig) {
                        try {
                            ...
                            console.warn(chalk.yellow(`\n⚠️ PostCSS/Tailwind processing failed for ${id}:`), err.message);
                        }
                    }
    ...
    function getProxyCode(dep, resolvedPath) {
        ...
        if (dep === 'react') {
            return `import * as m from ${resolvedPathEscaped}; export const { ${reactKeys.join(', ')} } = m; export default (m.default || m);`;
        }
        else if (dep === 'react-dom/client') {
            return `import * as m from ${resolvedPathEscaped}; export const { ${reactDomClientKeys.join(', ')} } = m; export default (m.default || m);`;
        }
        else if (dep === 'react-dom') {
            return `import * as m from ${resolvedPathEscaped}; export const { ${reactDomKeys.join(', ')} } = m; export default (m.default || m);`;
        }
        else if (dep === 'react/jsx-runtime' || dep === 'react/jsx-dev-runtime') {
            return `import * as m from ${resolvedPathEscaped}; export const { ${jsxRuntimeKeys.join(', ')} } = m; export default (m.default || m);`;
        }
        else {
            try {
                const m = require(resolvedPath);
                const keys = [...new Set([...Object.keys(m), ...Object.getOwnPropertyNames(m)])].filter((k) => k !== 'default' &&
                    k !== '__esModule' &&
                    k !== 'constructor' &&
                    k !== 'prototype' &&
                    k !== 'arguments' &&
                    ...
                    typeof k === 'string');
                if (keys.length > 0) {
                    return `import * as m from ${resolvedPathEscaped}; export const { ${keys.join(', ')} } = m; export default (m.default || m);`;
                }
                else {
        ...
        try {
            ...
            for (const file of bundleFiles) {
                if (file.endsWith('.js')) {
                    ...
                    content = content.replace(/\b(from|import)\s+['"]\.\/([^'"]+)['"]/g, (_m, type, chunk) => `${type} "/@modules/${chunk}"`);
...
`;
    ...
    app.use((async (req, res, next) => {
        ...
        if (assets.includes(ext)) {
            ...
            const isImport = req.url?.includes('?import') ||
                secFetchDest === 'script' ||
        ...
        try {
            ...
            transformedCode = transformedCode
                ...
                .replace(/(\bimport\s+['"])([^'".\/][^'"]*)(['"])/gm, (_m, start, dep, end) => `${start}/@modules/${dep}${end}`)
    ...
    app.use((async (req, res, next) => {
        ...
        const shouldServeIndex = url === '/' || url === '/index.html' || (!isInternal && !hasExtension) || isHtmlRequest;
        if (!shouldServeIndex)
            return next();
