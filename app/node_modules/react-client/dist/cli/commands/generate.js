...
async function getProjectContext() {
    ...
    if (!fs.existsSync(srcDir)) {
        console.error(chalk.red('❌ Error: Could not find "src" directory. Make sure you are in the project root.'));
        process.exit(1);
    }
    const isTS = fs.existsSync(path.resolve(root, 'tsconfig.json')) ||
...
`;
    ...
    const hasRouter = pkg.dependencies?.['react-router-dom'] || pkg.devDependencies?.['react-router-dom'];
    if (!hasRouter) {
        const res = await prompts({
            type: 'confirm',
            name: 'install',
            message: chalk.yellow('react-router-dom is not installed. Would you like to install it and set up routing?'),
            initial: true,
        });
        if (res.install) {
            ...
            try {
                ...
                console.error(chalk.red('❌ Failed to install react-router-dom. Please install it manually.'));
            }
        }
        else {
...
async function setupInitialRouter(root, srcDir, isTS, firstPage) {
    const ext = isTS ? 'tsx' : 'jsx';
    ...
    if (fs.existsSync(mainFile)) {
        ...
        if (!mainContent.includes('BrowserRouter')) {
            ...
            mainContent = mainContent.replace(/<App\s*\/>/g, '<BrowserRouter>\n      <App />\n    </BrowserRouter>');
...
async function registerRoute(root, srcDir, isTS, componentName, routePath) {
    const ext = isTS ? 'tsx' : 'jsx';
    const appFile = path.resolve(srcDir, `App.${ext}`);
    ...
    if (appContent.includes('</nav>')) {
        if (!appContent.includes(`to="${routePath}"`)) {
            appContent = appContent.replace(/(\n\s+)<\/nav>/, `$1  <Link to="${routePath}" style={{ marginRight: '1rem' }}>${componentName}</Link>$1</nav>`);
        }
    }
    // Add Route
    if (!appContent.includes(`element={<${componentName} />`)) {
        appContent = appContent.replace(/(\n\s+)<\/Routes>/, `$1  <Route path="${routePath}" element={<${componentName} />} />$1</Routes>`);
        await fs.writeFile(appFile, appContent, 'utf8');
...
export async function component(name) {
    ...
    if (fs.existsSync(componentFile)) {
        console.error(chalk.red(`❌ Error: Component "${componentName}" already exists at ${componentFile}`));
        process.exit(1);
    }
    // 3️⃣ Generate the component
...
`
        : ''}export default function ${componentName}(${isTS ? `{ children }: ${componentName}Props` : '{ children }'}) {
  return (
